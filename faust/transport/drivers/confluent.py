...
class ConfluentConsumerThread(ConsumerThread, BrokerCredentialsMixin):
    ...
    def _create_client_consumer(
        ...
        credentials: Mapping = None) -> _Consumer:
        conf = self.app.conf
        self.log.info(f'the bootstrap servers are {transport.url} and port is {transport.default_port}')
...
class ProducerThread(QueueServiceThread, BrokerCredentialsMixin):
    ...
    async def on_start(self) -> None:
        ...
        self.log.info(f'the bootstrap servers are {self.transport.url} and port is {self.transport.default_port}')
